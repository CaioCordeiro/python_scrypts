...
class AG:
	...
	def disCidades(self, cidades):																					#GERA DISTANCIA ENTRE CIDADES , MATRIZ DE ADJACENCIA
		for i in cidades:
			self.distancia[i] = {}
			for j in cidades:
				if  j not in self.distancia[i]: 																	#SE J NAO ESTA NO DICIONARIO DA CIDADE I VOU INCLUI-LA
					if j in self.distancia and j!=i:																#SE CIDADE J ESTA NO DICIONARIO (JA TENHO VALOR) E J!=I (NAO E A MESMA CIDADE)
	...
	def __header(self):
		try:
			...
			print("")
			pop = input("IMPRIMIR POPULACAO (S/N):  ")
			if pop.lower() =='s':
...
if __name__ == "__main__":
	try:
		...
		ger = eval(input("NUMERO DE GERACOES: "))
		pop = eval(input("TAMANHO DA POPULACAO: "))
		citys = eval(input("NUMERO DE CIDADES: "))
		c = []
